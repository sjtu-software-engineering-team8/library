      ...
      <div style="overflow:hidden;padding-top:100px">
        ...
        <div class="cssfx">
          <input
            ...
            v-model="usercookie"
            @keyup.enter="confirmuser"
          />
          <span></span>
        </div>
      </div>
      <div>
        <button @click="confirmuser">确认</button>
      </div>
    </div>
    <div v-else>
...
export default {
  ...
  methods: {
      ...
      this.$http
        ...
        .then(function(result) {
          if (result.body.status === 0) {
            ...
            this.$store.state.No = result.body.no;
            this.check();
    ...
    check() {
      if (this.$store.state.cookie === "") this.$store.state.flag = true;
      else this.$store.state.flag = false;
    },
    created() {
      this.check();
    }
