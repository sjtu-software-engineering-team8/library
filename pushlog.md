...
2. 新增以下接口：http://127.0.0.1/credit/getcredit接口获取本人积分的，http://1270.0.1/credit/getall获取积分排名降序排列，http://127.0.0.1/credit/insert用于测试的时候批量生成随机积分，用于生成测试数据

3. 要成功运行后端部分，需要进行一下步骤:     1. 重新数据库迁移     2. 运行测试数据生成集中的 user.py  desk.py   3. 然后在启动项目后在浏览器中输入http://127.0.0.1/credit/insert，即可完成原始数据的生成
